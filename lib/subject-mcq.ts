...
export async function getPublicSubjectQuestions(subject: string, accessToken: string): Promise<PublicSubjectQuestion[]> {
  ...
  const response = await fetch(`${url}/rest/v1/SUBJECT_MCQ_QUESTIONS?${params.toString()}`, { headers: { apikey: key, Authorization: `Bearer ${accessToken}`, "Accept-Profile": "public" }, cache: "no-store" });
