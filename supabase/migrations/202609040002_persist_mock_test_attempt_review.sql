-- Persist the server-calculated review with each submitted attempt so results
-- can be safely reloaded after the browser session ends.
...
begin
  ...
  insert into public."TEST_ATTEMPTS" (user_id, test_id, test_title, score, percentage, answers, review)
  values (auth.uid(), p_test_id, test_name, correct_count, percentage, submitted_answers, question_review)
