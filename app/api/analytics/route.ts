...
import { getStudentAttemptAnalytics } from "@/lib/attempt-analytics";
...
export async function GET() {
  ...
  if (!user || !accessToken) {
    return NextResponse.json({ error: "Please log in to view your analytics." }, { status: 401 });
  }

  try {
    const analytics = await getStudentAttemptAnalytics(accessToken, user.id);
    ...
    const message = error instanceof Error ? error.message : "Unable to load your analytics.";
