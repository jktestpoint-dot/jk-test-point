...
export default function Contact() {
  ...
  return <section className="container-page py-14"><form onSubmit={submit} className="card mx-auto max-w-xl"><p className="eyebrow">Contact us</p><h1 className="mt-2 text-3xl font-bold">We&apos;d love to hear from you</h1><p className="mt-2 text-sm text-stone-500">Questions, feedback or support — send us a note.</p><input required name="name" autoComplete="name" className="input mt-6" placeholder="Your name"/><input required name="email" type="email" autoComplete="email" className="input mt-3" placeholder="Email address"/><textarea required name="message" className="input mt-3 min-h-32" placeholder="How can we help?"/>{sent ? <p className="mt-4 rounded-xl bg-emerald-50 p-3 text-sm text-emerald-700">Thank you! Our team will get back to you shortly.</p> : <button disabled={sending} className="btn-primary mt-5">{sending ? "Sending…" : "Send message"}</button>}{error && <p className="mt-4 rounded-xl bg-rose-50 p-3 text-sm text-rose-700">{error}</p>}</form></section>;
}
