...
export default function Dashboard() {
  ...
  return <section className="container-page py-10">
    ...
    <div className="mt-7 grid gap-6 lg:grid-cols-[1.5fr_1fr]"><div className="card"><div className="flex items-center justify-between gap-3"><h2 className="font-bold">Performance trend</h2>{trend.length>1&&<span className="text-xs text-stone-400">Oldest to newest</span>}</div>{loading?<p className="mt-6 text-sm text-stone-500" role="status">Loading performance data…</p>:error||!dashboard?<p className="mt-6 text-sm text-stone-500">Performance data could not be loaded. Please refresh to try again.</p>:trend.length===0?<p className="mt-6 text-sm text-stone-500">No attempt data yet. Complete a test to see your progress.</p>:<><div className="mt-6 flex h-44 items-end justify-between gap-3">{trend.map((attempt,index)=><div className="flex h-full flex-1 flex-col items-center justify-end gap-2" key={attempt.id}><span className="text-xs font-semibold text-brand-700">{Math.round(attempt.percentage)}%</span><div className="min-h-[3px] w-full rounded-t bg-brand-500" style={{height:`${Math.max(attempt.percentage,2)}%`}} title={`${attempt.test_title || "Attempt"}: ${Math.round(attempt.percentage)}%`}/><span className="text-xs text-stone-400">T{index+1}</span></div>)}</div>{trend.length===1&&<p className="mt-4 text-sm text-stone-500">Complete one more test to start comparing your performance.</p>}</>}</div><div className="card"><h2 className="font-bold">Recent tests</h2>{attempts.length?<div className="mt-4 space-y-4 text-sm">{attempts.slice(0,3).map((attempt)=><div className="flex justify-between" key={attempt.id}><span>{attempt.test_title||"Mock test"}</span><b className="text-emerald-600">{Math.round(attempt.percentage)}%</b></div>)}</div>:<p className="mt-4 text-sm text-stone-500">No completed tests yet.</p>}<Link className="btn-primary mt-6 w-full" href="/mock-tests">Take a new test</Link></div></div>
