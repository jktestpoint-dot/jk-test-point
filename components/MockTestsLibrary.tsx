...
export default function MockTestsLibrary({ initialTests, initialError = "" }: { initialTests: CatalogTest[]; initialError?: string }) {
  ...
  const grouped = useMemo(() => MAIN_CATEGORIES.map((main) => ({ main, tests: tests.filter((test) => test.main_category === main).sort((a, b) => a.subcategory.localeCompare(b.subcategory) || a.title.localeCompare(b.title)) })), [tests]);
  ...
  return <section className="container-page section-space"><div className="rounded-2xl border border-brand-100 bg-gradient-to-br from-brand-50 to-white p-6 sm:p-8"><p className="eyebrow">Test library</p><h1 className="mt-2 text-3xl font-bold sm:text-4xl">Find your next mock test</h1><p className="mt-3 max-w-2xl text-sm leading-6 text-stone-600">Browse by exam category or search for the practice test you need.</p><div className="mt-7 grid gap-3 md:grid-cols-[1fr_auto]"><input className="input bg-white" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search tests, exams or topics..." /><select className="input md:w-56" value={category} onChange={(event) => setCategory(event.target.value)} aria-label="Filter by category"><option value="All">All categories</option>{grouped.map((group) => <optgroup key={group.main} label={group.main}>{Array.from(new Set(group.tests.map((test) => test.subcategory))).map((subcategory) => <option key={subcategory} value={`${group.main}::${subcategory}`}>{subcategory}</option>)}</optgroup>)}</select></div><div className="mt-4 flex flex-wrap gap-2" aria-label="Quick category filters"><button type="button" onClick={() => setCategory("All")} className={`rounded-full border px-3 py-1.5 text-sm font-semibold transition ${activeMainCategory === "All" ? "border-brand-600 bg-brand-600 text-white" : "border-brand-100 bg-white text-stone-600 hover:border-brand-300 hover:text-brand-700"}`}>All</button>{availableCategories.map((main) => <button type="button" key={main} onClick={() => setCategory(main)} className={`rounded-full border px-3 py-1.5 text-sm font-semibold transition ${activeMainCategory === main ? "border-brand-600 bg-brand-600 text-white" : "border-brand-100 bg-white text-stone-600 hover:border-brand-300 hover:text-brand-700"}`}>{main}</button>)}</div></div>{initialError && <div className="card mt-6 text-center text-rose-700">{initialError}</div>}{!initialError && category === "All" && !query && <div className="mt-10 space-y-10" aria-label="Mock tests by category">{grouped.map((group) => <section key={group.main}><div className="flex items-center gap-3"><span className="h-px flex-1 bg-brand-100" /><h2 className="text-lg font-bold text-brand-700">{group.main}</h2><span className="h-px flex-1 bg-brand-100" /></div>{group.tests.length ? <div className="mt-5 grid gap-5 md:grid-cols-2 lg:grid-cols-3">{group.tests.map((test) => <TestCard key={test.id} test={test} />)}</div> : <div className="card mt-5 text-sm text-stone-500">No published tests in this category yet.</div>}</section>)}</div>}{!initialError && (category !== "All" || query) && <><p className="mt-6 text-sm text-stone-500">{shown.length} test{shown.length !== 1 ? "s" : ""} found</p><div className="mt-4 grid gap-5 md:grid-cols-2 lg:grid-cols-3">{shown.map((test) => <TestCard key={test.id} test={test} />)}</div>{!shown.length && <div className="card mt-5 text-center text-stone-500">No tests match these filters. Try a different search.</div>}</>}</section>;
}
