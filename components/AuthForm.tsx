...
declare global { interface Window { turnstile?: { render: (element: HTMLElement, options: { sitekey: string; callback: (token: string) => void; "expired-callback": () => void; "error-callback": () => void }) => string; reset: (widgetId?: string) => void }; } }
...
export function AuthForm({ register = false }: { register?: boolean }) {
  ...
  const authLink = (path: string) => nextPath ? `${path}?next=${encodeURIComponent(nextPath)}` : path;
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      widgetId.current = window.turnstile.render(captchaRef.current, { sitekey: turnstileSiteKey, callback: setTurnstileToken, "expired-callback": () => setTurnstileToken(""), "error-callback": () => setTurnstileToken("") });
    };
    ...
    return () => { if (widgetId.current && window.turnstile) window.turnstile.reset(widgetId.current); widgetId.current = undefined; };
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!register) { const session = await supabase.auth.signInWithPassword({ email: payload.email, password: payload.password, turnstileToken }); const role = await fetch("/api/auth/role", { cache: "no-store" }); const roleBody = await role.json().catch(() => ({ admin: false })) as { admin?: boolean }; window.dispatchEvent(new Event("jk-auth-change")); router.push(nextPath || (roleBody.admin ? "/admin" : "/dashboard")); router.refresh(); return; }
      const response = await fetch("/api/auth/register", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...payload, turnstileToken }) });
      ...
      window.dispatchEvent(new Event("jk-auth-change")); router.push(nextPath || "/dashboard"); router.refresh();
  ...
  return <section className="container-page py-14"><form onSubmit={submit} className="card mx-auto max-w-md"><p className="eyebrow">{register ? "Create account" : "Welcome back"}</p><h1 className="mt-2 text-3xl font-bold">{register ? "Start preparing today" : "Login to your account"}</h1>{register && <label className="mt-6 block text-sm font-medium">Full name<input required name="name" className="input mt-2" placeholder="Your name" autoComplete="name" /></label>}<label className="mt-4 block text-sm font-medium">Email address<input required name="email" type="email" className="input mt-2" placeholder="you@example.com" autoComplete="email" /></label><label className="mt-4 block text-sm font-medium">Password<input required name="password" minLength={8} type="password" className="input mt-2" placeholder="At least 8 characters" autoComplete={register ? "new-password" : "current-password"} /></label>{!register && <Link href={authLink("/forgot-password")} className="mt-3 block text-right text-sm font-semibold text-brand-700">Forgot Password?</Link>}<div ref={captchaRef} className="mt-5 flex min-h-[65px] w-full justify-center overflow-hidden" />{!turnstileSiteKey && <p className="mt-2 text-sm text-rose-600">Please complete the security check.</p>}{register && <p className="mt-3 text-xs leading-5 text-stone-500">We will verify this email with a one-time code before activating your account.</p>}{error && <p className="mt-3 text-sm text-rose-600">{error}</p>}<button disabled={loading} className="btn-primary mt-6 w-full">{loading ? "Please wait..." : register ? "Create account" : "Login"}</button><p className="mt-5 text-center text-sm text-stone-500">{register ? "Already have an account?" : "New to JK Test Point?"} <Link className="font-semibold text-brand-600" href={authLink(register ? "/login" : "/register")}>{register ? "Login" : "Create an account"}</Link></p></form></section>;
}
