...
export const metadata: Metadata = {
  ...
  openGraph: { title: "Paid MCQ Practice", description: "Browse subject-wise paid MCQ question banks, current question counts and practice availability.", url: "/mcq-practice", type: "website" },
  twitter: { card: "summary", title: "Paid MCQ Practice", description: "Browse subject-wise paid MCQ question banks, current question counts and practice availability." },
};
...
export default async function PaidMcqPractice() {
  const subjects = await Promise.all(MCQ_PRACTICE_SUBJECTS.map(async (subject) => ({
    ...subject,
    mcqCount: await getSubjectQuestionCount(subject.id).catch(() => 0),
  })));
  return <section className="container-page py-12 text-center">
    ...
    <div className="mt-10 grid gap-5 text-left sm:grid-cols-2 lg:grid-cols-4">
      {subjects.map((subject) => <article className="card" key={subject.id}>
