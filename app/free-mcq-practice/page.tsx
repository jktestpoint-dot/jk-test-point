...
export default async function FreeMcqPracticePage() {
  ...
  return <section className="container-page section-space"><div className="mx-auto max-w-5xl"><div className="page-intro"><p className="eyebrow">No purchase required</p><h1 className="mt-2 text-3xl font-bold sm:text-4xl">Free MCQ Practice</h1><p className="mt-4 max-w-2xl text-lg leading-7 text-stone-600">Choose a free subject and start practicing with selected questions.</p></div>{subjects.length ? <div className="mt-8 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">{subjects.map((subject) => <article className="card card-lift" key={subject.id}><div className="flex items-start justify-between gap-3"><h2 className="font-bold text-stone-800">{subject.name}</h2><span className="rounded-full bg-brand-50 px-2.5 py-1 text-xs font-bold text-brand-700">Free</span></div><p className="mt-3 text-sm text-stone-500">{subject.freeQuestionCount} free MCQ{subject.freeQuestionCount === 1 ? "" : "s"} available</p><Link href={"/free-mcq-practice/" + subject.id} className="btn-primary mt-5 w-full !px-4 !py-2">Start Free Practice</Link></article>)}</div> : <div className="mt-8 rounded-xl border border-dashed border-brand-200 bg-brand-50/60 p-6 text-center text-sm text-stone-600">Free practice sets are being prepared. Please check back soon.</div>}</div></section>;
}
