-- Extend the existing subject-MCQ system without changing existing questions,
-- attempts, prices, or subject data.
do $$
...
begin
  for constraint_name in
    ...
    where n.nspname = 'public'
      and t.relname = 'SUBJECT_MCQ_QUESTIONS'
      and c.contype = 'c'
      and pg_get_constraintdef(c.oid) ilike '%subject%'
  loop
    execute format('alter table public."SUBJECT_MCQ_QUESTIONS" drop constraint %I', constraint_name);
  end loop;
end;
$$;
...
alter table public."SUBJECT_MCQ_QUESTIONS"
  ...
  check (subject in ('accountancy', 'mathematics', 'statistics', 'economics', 'computer'));
...
begin
  for constraint_name in
    ...
    join pg_namespace n on n.oid = t.relnamespace
    where n.nspname = 'public'
      and t.relname = 'SUBJECT_MCQ_ATTEMPTS'
      and c.contype = 'c'
      and pg_get_constraintdef(c.oid) ilike '%subject%'
  loop
    execute format('alter table public."SUBJECT_MCQ_ATTEMPTS" drop constraint %I', constraint_name);
  end loop;
end;
$$;

alter table public."SUBJECT_MCQ_ATTEMPTS"
  add constraint subject_mcq_attempts_subject_allowed_check
  check (subject in ('accountancy', 'mathematics', 'statistics', 'economics', 'computer'));
...
begin
  ...
  if p_subject not in ('accountancy', 'mathematics', 'statistics', 'economics', 'computer') then raise exception 'Invalid subject'; end if;
