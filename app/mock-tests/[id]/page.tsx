...
export default async function TestDetails({ params }: { params: { id: string } }) {
  ...
  const hasEntitlement = Boolean(test.price > 0 && token && user && await hasActiveMockEntitlement(token, test.id).catch(() => false));
  ...
  return <section className="container-page section-space"><div className="mx-auto max-w-4xl">
    ...
    <div className="mt-6 grid gap-6 lg:grid-cols-[1fr_auto]"><article className="card"><h2 className="text-lg font-bold">Instructions</h2><ul className="mt-4 list-disc space-y-3 pl-5 text-sm leading-6 text-stone-600"><li>Read every question carefully before selecting an answer.</li><li>You may move between questions using the palette.</li><li>Use Mark for Review to revisit a question before submission.</li><li>The timer begins as soon as you start the test.</li></ul></article><div className="card flex flex-col justify-center sm:min-w-56"><p className="text-sm text-stone-500">Ready to begin?</p>{test.price > 0 ? <SubjectCheckoutButton mockTestId={test.id} subjectName={test.title} hasEntitlement={hasEntitlement} isAuthenticated={Boolean(user && token)} /> : <Link href={`/mock-tests/${test.id}/attempt`} className="btn-primary mt-4 w-full">Start Test →</Link>}<Link href={`/mock-tests/${test.id}/leaderboard`} className="btn-secondary mt-3 w-full">Leaderboard</Link></div></div>
  </div></section>;
}
