...
export async function GET(request: NextRequest, { params }: { params: { subject: string } }) {
  ...
  if (!attemptId) return NextResponse.json({ error: "Select a submitted free practice result." }, { status: 400 });
  try {
    ...
    if (!response.ok) return NextResponse.json({ error: "Unable to load this free practice result." }, { status: 502 });
    return withRefreshedSession(NextResponse.json({ data: Array.isArray(rows) ? rows[0] ?? null : null }), refreshed);
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : "Unable to load this free practice result." }, { status: 502 });
  }
}
...
export async function POST(request: NextRequest, { params }: { params: { subject: string } }) {
  ...
  try {
    ...
    if (!Array.isArray(answers)) return NextResponse.json({ error: "Invalid free practice attempt." }, { status: 400 });
    const { url, key } = getSupabaseConfig();
    ...
    if (!response.ok) return NextResponse.json({ error: data?.message || "Unable to submit free practice." }, { status: 400 });
    return withRefreshedSession(NextResponse.json({ data }, { status: 201 }), refreshed);
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : "Unable to submit free practice." }, { status: 502 });
  }
}
